...
function civicrm_api3_donor_link_step1a($params) {
  ...
  $querySelect = 'SELECT id, contact_id, group_id FROM civicrm_subscription_history '
    . 'WHERE status = %1 AND date >= %2 AND contact_id = 24587';
  $paramsSelect = array(
  ...
  while ($daoSelect->fetch()) {
    ...
    checkToBeDeleted($daoSelect->contact_id, $daoSelect->group_id) == TRUE) {
...
function checkToBeDeleted($contactId, $groupId) {
  $query = 'SELECT COUNT(*) AS count_added WHERE contact_id = %1 AND group_id = %2 AND status = %3 AND date >= %4';
  $params = array(
    1 => array($contactId, 'Positive'),
    2 => array($groupId, 'Positive'),
    3 => array('Added', 'String'),
    4 => array('2014-01-01', 'String'));
